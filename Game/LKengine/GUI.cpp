...
namespace LKengine {
	...
	CEGUI::OpenGL3Renderer* LKengine::GUI::m_renderer = nullptr;
	...
	{
		// Check if the renderer and system were already initialised
		if (m_renderer == nullptr)
		{
			m_renderer = &CEGUI::OpenGL3Renderer::bootstrapSystem();

			// Static cast so if we can't cast it, the comiler will complain to us rather than crash and not know why
		...
		// Set up context so it can use our renderer
	...
	*************************************************************************/
	CEGUI::Key::Scan SDLKeyToCEGUIKey(SDL_Keycode key)
	{
		...
		// find these as these are the old ones
	...
	/************************************************************************
	Convert SDL mouse inputs to CEGUI mouse inputs
	*************************************************************************/
	...
	void GUI::onSDLEvent(SDL_Event& evnt)
	{
		...
		case SDL_TEXTINPUT:
			...
			{
				// Bit shift the input until the null character is hit (only iterate once on English keyboard)
	...
	CEGUI::Window* GUI::createWidget(const std::string& type, const glm::vec4& destRectPercentage, const glm::vec4& destRecPixels, const std::string& name /* = "" */)
	{
		CEGUI::Window* newWindow = CEGUI::WindowManager::getSingleton().createWindow(type, name);
		m_root->addChild(newWindow);

		setWidgetDestRect(newWindow, destRectPercentage, destRecPixels);
		
		return newWindow;
	}


	void GUI::setWidgetDestRect(CEGUI::Window* widget, const glm::vec4& destRectPercentage, const glm::vec4& destRecPixels)
	{
		widget->setPosition(CEGUI::UVector2(CEGUI::UDim(destRectPercentage.x, destRecPixels.x), CEGUI::UDim(destRectPercentage.y, destRecPixels.y)));
		widget->setSize(CEGUI::USize(CEGUI::UDim(destRectPercentage.z, destRecPixels.z), CEGUI::UDim(destRectPercentage.w, destRecPixels.w)));
