...
namespace LKengine {
	...
	ShaderProgram::ShaderProgram() : m_numAttributes(0), m_programID(0), m_vertexShaderID(0), m_fragmentShaderID(0)
	{
	...
	void ShaderProgram::linkShaders()
	{
		...
		if (isLinked == GL_FALSE)
		{
			...
			std::printf("%s \n", &(errorLog[0]));
			fatalError("Shader failed to line!");
		}
	...
	void ShaderProgram::compileShader(const char* source, const std::string& name, GLuint id)
	{
		...
		if (success == GL_FALSE)
		{
			...
			// provides the infolog in whatever manor you deem best.
			// Exit with failure.
