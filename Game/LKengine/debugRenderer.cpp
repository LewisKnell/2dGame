...
namespace LKengine {
	...
	void DebugRenderer::drawCircle(const glm::vec2& centre, const ColorRGBA8& color, float radius)
	{
		...
		for (int i = 0; i < NUM_VERTS; i++)
		{
			...
			m_verts[start + i].position.x = cos(angle) * radius + centre.x;
			m_verts[start + i].position.y = sin(angle) * radius + centre.y;
